...
- Q: Which date formats should Phase 2 accept from CSV imports? -> A: Accept ISO dates and common Notion date text like `April 15, 2026`.
- Q: Which delimiter should Phase 2 use for tags in CSV imports? -> A: Tags are comma-separated, like `deep work, writing`.
- Q: Which status vocabulary should Phase 2 ImportRun records use? -> A: Use Phase 1 statuses: `pending`, `processing`, `completed`, `completed_with_errors`, `failed`.
- Q: Should Phase 2 retain raw CSV files after import? -> A: Do not retain raw CSV; keep metadata and row outcomes only.
- Q: When should Phase 2 create ImportRun records? -> A: Create ImportRun only when the user confirms import.
...
5. **Given** the user previews a CSV file without confirming import, **When** the preview completes, **Then** no ImportRun record is created.
...
1. **Given** the user confirms an import, **When** processing begins, **Then** the system creates an import run with a status that reflects pending or active processing.
...
- If a row has a missing date, a date that is neither ISO format nor common Notion date text, missing task, missing category, missing time spent, zero time spent, negative time spent, or non-whole-number time spent, the row must be invalid.
- If optional `tags` is missing, blank, or contains only blank values, the row must use an empty tag array.
- If optional `tags` contains comma-separated values, each comma-separated value must be normalized into a separate tag.
...
- If a user requests preview only, no tracking records or ImportRun records are created.
...
- If a user reviews import history later, the system must provide import metadata and row outcomes without requiring retention of the full raw CSV file.
...
- **FR-007a**: The system MUST accept CSV row dates in ISO format and common Notion date text such as `April 15, 2026`.
- **FR-007b**: Preview behavior MUST NOT create ImportRun records; ImportRun records MUST be created only when the user confirms import.
...
- **FR-009**: The system MUST split imported tags on commas and convert them into an array of unique, non-empty normalized values.
...
- **FR-015**: The system MUST make import status available after confirmation using the shared run statuses `pending`, `processing`, `completed`, `completed_with_errors`, and `failed`.
...
- **FR-023**: The system MUST preserve import run history after completion or failure so the user can review what happened later without retaining the full raw CSV file.
- **FR-023a**: The system MUST retain import metadata and row outcomes only; full raw CSV file contents MUST NOT be retained after import history is recorded.
...
- **Import Row Outcome**: A row-level trace for invalid, skipped, or failed rows with row reference, outcome type, and reason. It does not require storing the full raw CSV file.
...
- Tags in the CSV are provided as comma-separated text values.
