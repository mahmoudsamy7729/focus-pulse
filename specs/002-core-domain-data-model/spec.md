...
1. **Given** an AI insight run analyzes a week of daily logs, **When** the run is recorded, **Then** it identifies its target period and the source daily records used for analysis.
...
- **FR-017**: The model MUST support later import deduplication based on date, task name, and time spent without requiring duplicate source records to be stored as separate completed tasks.
- **FR-018**: The model MUST define AIInsightRun as a traceable analysis attempt with target period, source daily records, processing status, start time, completion time when available, and failure details when applicable.
...
- **AIInsightRun**: A traceable AI analysis attempt that records target period, source daily records, status, timing, rerun history, and failure details without mutating source tracking records.
