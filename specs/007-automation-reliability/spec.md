...
- If a schedule is due more than once because the scheduler was offline, the system must process at most one run per schedule window and record skipped or missed windows clearly.
...
- **FR-004**: Scheduled imports MUST use only approved import sources that satisfy the existing CSV import contract and MUST NOT introduce Notion API synchronization or arbitrary external connectors in this phase.
...
- **FR-026**: Phase 6 MUST cover scheduled imports, scheduled AI analysis runs, retry/recovery behavior, logs, auditability, and automation status/history only; report export, Notion API synchronization, AI chat, dashboard redesign, and new AI recommendation logic are outside this phase.
...
- **Scheduled Import Run**: A traceable import run started by an automation schedule rather than by an immediate manual confirmation.
- **Scheduled AI Analysis Run**: A traceable AI analysis run started by an automation schedule for a completed day or week.
- **Automation Run History**: The reviewable record of schedule evaluations, created runs, skipped windows, retries, completions, partial completions, and failures.
...
- **SC-001**: 100% of due active automation schedules produce exactly one started, skipped, or failed history entry for each schedule window.
...
- **SC-011**: A reviewer can verify in under 10 minutes that Phase 6 does not add report export, Notion API synchronization, AI chat, dashboard redesign, arbitrary external import connectors, raw CSV retention, or new AI recommendation logic.
...
- Scheduled imports automate only supported import inputs that already satisfy the approved CSV import contract; connector-specific synchronization, including Notion API synchronization, remains a future Phase 7 concern.
...
- Scheduled AI analysis automates Phase 4 analysis execution for completed day and week periods; it does not create AI chat behavior or new recommendation-generation logic.
