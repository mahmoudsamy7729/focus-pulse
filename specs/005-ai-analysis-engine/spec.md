...
As the project owner, I want AI-generated daily and weekly summaries from my saved tracking data, so I can understand what happened without manually reading every task, category, tag, and note.
...
1. **Given** a selected day has saved tracking records, **When** the user requests a daily analysis, **Then** the system creates a daily summary that reflects that day's total tracked time, task count, category and tag themes, and notable note context when notes exist.
...
3. **Given** the selected period has no saved tracking records, **When** analysis is requested, **Then** the system records a clear no-data outcome and does not produce fabricated summary text.
...
**Independent Test**: Can be tested with sample days and weeks containing repeated categories, tags, task themes, uneven daily totals, and mixed notes, then verifying that generated patterns include supporting evidence from the source data.
...
3. **Given** task notes are present, **When** analysis completes, **Then** note-derived observations are clearly distinguishable from duration, category, and tag observations.
...
1. **Given** the user requests analysis, **When** processing begins, **Then** a traceable analysis run exists with the selected period, analysis type, status, request time, and source data summary.
...
4. **Given** a failed run is retried, **When** retry processing completes, **Then** the retry outcome is linked to the original run and does not create duplicate current results.
...
- If the selected day or week has no saved tracking records, the system must record a no-data outcome and avoid producing AI-generated claims.
...
- If tags or notes are absent from some tasks, the analysis must still run using available task, duration, and category data.
- If a task has multiple tags, analysis evidence must make the multi-tag interpretation clear when tag totals are discussed.
- If a category, tag, task name, or note is unusually long, the analysis output must remain readable and must not drop the associated tracked-time evidence.
...
- If two analysis requests for the same period and type are made close together, the system must avoid conflicting current results and keep the run history understandable.
...
- If an analysis retry occurs after a transient failure, the retry must not duplicate output records or hide the earlier failed attempt.
...
- If a user reviews older analysis history, the system must make it clear which completed result is the latest successful result for the period and type.
...
- **FR-002**: The system MUST assemble a structured analysis input for the selected period that includes period boundaries, saved tasks, tracked durations, categories, tags, notes when present, and aggregate totals.
...
- **FR-004**: If a selected period has no saved tracking records, the system MUST create a no-data analysis outcome instead of generating summary, pattern, or insight claims.
- **FR-005**: The system MUST maintain named and versioned analysis instructions for daily summaries, weekly summaries, pattern detection, and behavior insights so outputs can be traced to the instruction set used.
...
- **FR-010**: Completed analysis outputs MUST include structured sections for summary, detected patterns, behavior insights, supporting evidence, limitations, and generated timestamp.
...
- **FR-013**: The system MUST store each analysis attempt as a traceable analysis run with analysis type, selected period, status, request time, requester, source record counts, input summary, instruction version, completion time when available, output when successful, retry count, and failure details when applicable.
...
- **FR-015**: Analysis runs MUST use stable statuses that distinguish waiting, in-progress, completed, completed with no data, and failed outcomes.
- **FR-016**: The system MUST retry transient analysis failures within a bounded retry policy and preserve each failed attempt's traceability.
- **FR-017**: Retrying an analysis run MUST NOT create duplicate current outputs for the same run.
- **FR-018**: The user MUST be able to rerun analysis for the same period and analysis type.
- **FR-019**: Rerunning analysis MUST create a new run and preserve previous completed and failed runs for audit history.
- **FR-020**: When multiple successful runs exist for the same period and type, the system MUST clearly identify the latest successful result as the current result.
...
- **FR-023**: Analysis history MUST remain reviewable by period, analysis type, status, and completion time.
...
- **AI Analysis Request**: A user-initiated request to analyze a specific day or week with a selected analysis type.
- **AIInsightRun**: A traceable analysis attempt with status, selected period, analysis type, request and completion timing, source data summary, instruction version, output, retry count, and failure details.
- **Analysis Input Snapshot**: The structured period data prepared for analysis, including source record counts, totals, categories, tags, notes when present, and enough source references to explain the output.
...
- **Supporting Evidence**: Source facts used to justify a summary, pattern, or insight, such as dates, durations, task counts, categories, tags, or note references.
...
- **Current Analysis Result**: The latest successful completed run for a period and analysis type, while older runs remain available as history.
...
- **SC-007**: Retrying the same failed analysis run creates zero duplicate current results while preserving the failed attempt history.
...
- **SC-009**: A user can identify the latest successful result, prior runs, run status, and failure details for a period in under 2 minutes.
...
- The latest successful run is the default current result for a period and analysis type.
